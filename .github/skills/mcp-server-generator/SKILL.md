...
- This project is part of a repo with multiple demo MCP servers
- Work ONLY inside `todo-app`
...
- Do not use deprecated `Server` for standard setups (only advanced low-level scenarios)
- Set server name and version
- Choose appropriate transport:
  - **HTTP**: `StreamableHTTPServerTransport` from `@modelcontextprotocol/sdk/server/streamableHttp.js` (requires `.js` extension with NodeNext)
  - **stdio**: `StdioServerTransport` from `@modelcontextprotocol/sdk/server/stdio.js`
- For HTTP: set up Express, handle `POST /mcp`, `GET /mcp`, and `DELETE /mcp`; use `sessionIdGenerator: undefined` for stateless mode
- For HTTP stateless: create a fresh `McpServer` + `StreamableHTTPServerTransport` per request, then call `transport.handleRequest(req, res, req.body)`
- For stdio: use `StdioServerTransport` directly; avoid writing to stdout
- Never mix stdio and HTTP transports in the same server process

### Tool Implementation
- Use `registerTool()` method with descriptive names
- Define schemas using zod for input and output validation
- Always provide an explicit `inputSchema` for every tool
- For no-argument tools, use `z.object({})` as the input schema
- Prefer destructured handler arguments (e.g., `async ({ task }) => { ... }`)
- Keep a centralized tool-name-to-function mapping for consistent routing
